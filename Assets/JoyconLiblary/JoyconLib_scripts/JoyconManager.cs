...
public class JoyconManager: MonoBehaviour
{
    ...
    public static JoyconManager Instance
    {
        get
		{
			if (instance == null) instance = FindObjectOfType<JoyconManager>();
			return instance;
		}
    }
    ...
    //CI0329 OnDestroy追加 + OnApplicationQuit側にhid_exit追加
    void OnDestroy()
    {
        for (int i = 0; i < j.Count; ++i)
        {
    ...
    {
        for (int idx = 0; idx < j.Count; ++idx)
        {
            j[idx].Detach();
        }
        Debug.Log("exit:" + HIDapi.hid_exit());


        Debug.Log("init:" + HIDapi.hid_init());
        int i = 0;
        j.Clear();
        bool isLeft = false;
